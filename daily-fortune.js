...
function renderDailyFortune(kind, idx, label) {
  ...
  const rank = 1 + scores.filter((s, j) => s > score || (s === score && j < idx)).length;
  const rng = mulberry32(hashSeed(dateStr + "|" + kind + "-detail|" + idx));
  const stars = score >= 92 ? 5 : score >= 80 ? 4 : score >= 66 ? 3 : 2;
  ...
  document.getElementById("msg").textContent = DF_MSGS[Math.floor(rng() * DF_MSGS.length)];
